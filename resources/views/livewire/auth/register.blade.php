            ...
            <div class="container z-10">
                <div class="flex flex-wrap justify-center -mx-3">
                    <div class="w-full max-w-full px-3 mx-auto mt-0 text-center lg:flex-0 shrink-0 lg:w-5/12">
                        ...
                        <p class="text-white">Créez un compte pour accéder à toutes les fonctionnalités.</p>
                    </div>
                </div>
            </div>
        </div>
                            ...
                            <form wire:submit.prevent="register">
                                ...
                                <div class="mb-4">
                                    <input wire:model="name" type="text"
                                        ...
                                        placeholder="Nom" name="name" aria-label="Name" aria-describedby="email-addon"
                                        required autofocus />
                                    @error('name')
                                ...
                                <div class="mb-4">
                                    <input wire:model="password" type="password"
                                        class="text-sm focus:shadow-soft-primary-outline leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding py-2 px-3 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:bg-white focus:text-gray-700 focus:outline-none focus:transition-shadow"
                                        placeholder="Mot de passe" name="password" aria-label="Password"
                                ...
                                <div class="min-h-6 pl-7 mb-0.5 block">
                                    <input wire:model="terms"
                                        class="relative float-left -ml-7 w-4.5 h-4.5 rounded-1.4 checked:bg-gradient-to-tl checked:from-blue-500 checked:to-violet-500 after:text-xxs after:font-awesome after:duration-250 after:ease-soft-in-out duration-250 relative float-left mt-1 cursor-pointer appearance-none border border-solid border-slate-150 bg-white bg-contain bg-center bg-no-repeat align-top transition-all after:absolute after:flex after:h-full after:w-full after:items-center after:justify-center after:text-white after:opacity-0 after:transition-all after:content-['\f00c'] checked:border-0 checked:border-transparent checked:bg-transparent checked:after:opacity-100"
                                        type="checkbox" />
                                    <label
                                        class="mb-2 ml-1 font-normal cursor-pointer select-none text-sm text-slate-700"
                                        for="terms"> J'accepte les <a href="javascript:;"
                                            class="font-bold text-slate-700">Termes et Conditions</a> </label>
                                    @error('terms')
                                        <p class="text-size-sm text-red-500">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="text-center">
                                    <button type="submit"
                                    class="inline-block w-full px-6 py-3 mt-6 mb-0 font-bold text-center text-white uppercase align-middle transition-all bg-transparent border-0 rounded-lg cursor-pointer shadow-soft-md bg-x-25 bg-150 leading-pro text-size-xs ease-soft-in tracking-tight-soft bg-gradient-cyan hover:scale-102 hover:shadow-soft-xs active:opacity-85">S'inscrire</button>
                                </div>
                                <p class="mt-4 mb-0 leading-normal text-sm">Vous avez déjà un compte ? <a
                                        href="{{ route('login') }}" class="font-bold text-slate-700">Se connecter</a>
                                </p>
                            </form>
                        </div>
                    </div>
